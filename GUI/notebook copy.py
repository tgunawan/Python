...
class SimpleNotebookApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Button(toolbar, text="Buat Tab", command=self.create_new_tab).pack(side="left", padx=(0, 6))
        ttk.Button(toolbar, text="Catatan Baru", command=self.new_note).pack(side="left", padx=6)
        ...
        ttk.Button(toolbar, text="Tutup Tab", command=self.close_active_tab).pack(side="left", padx=6)
        ...
        self.create_new_tab("Catatan 1")

    def create_new_tab(self, title=None):
        if title is None:
            title = f"Catatan {len(self.tabs) + 1}"
    ...
    def close_active_tab(self):
        if len(self.tabs) <= 1:
            messagebox.showwarning("Tidak Bisa", "Minimal harus ada satu tab.")
            return

        index = self.notebook.index("current")
        active_tab = self.tabs[index]
        if active_tab["text"].get("1.0", tk.END).strip():
            if not messagebox.askyesno("Tutup Tab", "Tab ini memiliki isi. Tutup tab ini?"):
                return
